...
public class GradeCalculatorUsingArrays {
  ...
  public static void main(String[] args) {
    ...
    /*
     * Accept the perfect quiz score: The perfect score must be greater than 0. The
     * computer
    ...
    do {
      ...
      if (perfectQuizScore <= 0) {
        System.out.println(
          "Invalid value! The perfect score must be greater than 0."
        );
      }
    } while (perfectQuizScore <= 0);
    /*
     * Accept the perfect exam score: The perfect score must be greater than 0. The
     * computer
    ...
    for (int x = 0; x < size; x++) {
      ...
      /*
       * Accept the quiz score of a student: The quiz score must not be greater than
       * the
       * perfect quiz score. The computer lets the user re-enter a score in case the
       * value
       * entered is invalid.
       */
      do {
      ...
      /*
       * Accept the exam score: The exam score must not be greater than the perfect
       * exam
       * score. The computer lets the user re-enter a score in case the value entered
       * is
       * invalid.
       */
      do {
      ...
      if (subjectGrade[x] > 99) subjectGrade[x] = 99;
      // Check if subject grade is lesst than 65 then reset it to 65
      if (subjectGrade[x] < 65) subjectGrade[x] = 65;
    } // end of for
    ...
    for (int gr = 1; gr < subjectGrade.length; gr++) {
      if (lowestGrade > subjectGrade[gr]) lowestGrade = subjectGrade[gr];
    }
    ...
    for (int h = 1; h < subjectGrade.length; h++) {
      if (highestGrade < subjectGrade[h]) highestGrade = subjectGrade[h];
    }
